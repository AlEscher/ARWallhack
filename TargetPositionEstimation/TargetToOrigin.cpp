...
int main(int argc, char *argv[]) {
    ...
    if((sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0){
        cout << "error conneccting to remote host";
        return -1;
    }
    ...
    while (cap.read(frame)) {
        ...
        for (size_t k = 0; k < contours.size(); k++) {
            ...
            estimateSquarePose(resultMatrix, (Point2f*)corners, 0.03);
            ...
            //serialize matrix
            for (int i = 0; i < 16 ; i++){
                strcpy((matrix + index), &to_string(resultMatrix[i])[0]);
                index += strlen(&to_string(resultMatrix[i])[0]);
                if(i != 15)
                    if(i%4 == 3)
                        *(matrix + index) = '/';
                    else
            ...
            send(sock, matrix, strlen(matrix), 0);
            // Added in Exercise 5 - End *****************************************************************
