...
static void getCoordinates(float* coordinates, float* resultMatrix, cv::Point2f* corners, float markerSize, float* tb, Mat* rot, bool verbose = false){
    ...
    if(verbose){
        ...
        cout <<"rotations in x, y , z (A to target): " << rx << " " << ry << " " << rz << "\n";
...
void calcAtoB(Mat* inv,float** rotf, float* rb){
...
int main(int argc, char *argv[]) {
    ...
    float rb[] = {0,0,0};
    float rotf[3][3];
    Mat AtoB(Size(3,3), CV_32F, rotf);
    
    char opt;
    while((opt = getopt(argc, argv, "t:r:hv")) != -1){
        switch (opt) {
            case 't': {
                int offs = 0;
                for(int i = 0; i < 3; i++){
    ...
    while (cap.read(frame)) {
        ...
        for (size_t k = 0; k < contours.size(); k++) {
            ...
            getCoordinates(targets[ind], (float*)resultMatrix, (Point2f*)corners, 0.03, tb, &AtoB, verbose);
            codes[ind++] = *code;
                if (ind  == 10){
                    break;
                }
